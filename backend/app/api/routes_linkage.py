...
TRAINING_OBJECTIVES: dict[str, dict[str, Any]] = {
    ...
    "low_risk_validation": {
        ...
        "reason": "DQN 在 27 个可审计资源组合中学习，适合做离散动作对照。",
    },
}
...
ACTION_REGISTRY: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": "读取最新策略产物并执行能碳 KPI 仿真评估。",
        "intent_aliases": ["run_policy_test", "策略测试", "运行策略测试", "训练后策略测试"],
        "keywords": ["策略测试", "训练后测试", "测试策略", "运行测试", "仿真策略", "最新policy"],
        "button_selector": "#btnPolicyTest",
        "button_label": "策略测试",
        "backend_request": {"method": "POST", "path": "/api/rl/simulate", "body": {"strategy_id": "auto:latest"}},
        "linked_system": "energy_carbon_rl",
        "requires_human_confirm": False,
    },
    {
...
def _build_training_config(instruction: str, payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        dataset_id = registered_dataset_id(
            str(payload.get("dataset_id") or payload.get("data_file") or "port_la_2025_monthly")
        )
...
def _start_training(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        raw_config["dataset_id"] = registered_dataset_id(
            str(raw_config.get("dataset_id") or raw_config.get("data_file") or "port_la_2025_monthly")
        )
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
