...
class PortDataset:
    ...
    @classmethod
    def load(cls, dataset: str | Path = "port_la_2025_monthly") -> "PortDataset":
        ...
        if not {"train", "test"}.issubset(set(frame["split"].astype(str))):
            raise ValueError("Dataset must contain both train and test splits")
        train_periods = set(frame.loc[frame["split"].astype(str) == "train", "period"].astype(str))
        test_periods = set(frame.loc[frame["split"].astype(str) == "test", "period"].astype(str))
        if train_periods & test_periods:
            raise ValueError("A period cannot appear in both train and test splits")
        if frame[list(TEXT_COLUMNS)].fillna("").astype(str).apply(lambda column: column.str.strip().eq("")).any().any():
        ...
        for name, value in environment_parameters.items():
            ...
            if numeric_value < 0 or (name == "fuel_kwh_per_liter" and numeric_value == 0):
    ...
    def evaluation_start_indices(self, split: str, episode_hours: int) -> list[int]:
        ...
        if self.temporal_mode == "sequential_rows":
            return list(range(0, len(frame), max(1, int(episode_hours))))
    ...
    def quality_report(self) -> dict[str, Any]:
        ...
        duplicate_mask = self.frame.duplicated(subset=["period", "split", "source_id"], keep=False)
        ...
        source_urls_valid = isinstance(source_urls, list) and bool(source_urls) and all(
            isinstance(value, str) and value.startswith(("https://", "http://")) for value in source_urls
        )
        ...
        if min(train_rows, test_rows) < 2:
            score -= 15
            warnings.append("insufficient_split_rows")
        score = max(0, score)
        grade = "A" if score >= 90 else "B" if score >= 75 else "C" if score >= 60 else "D"
        return {
            "status": "pass" if score >= 75 and not duplicate_rows and not missing_cells else "review",
            "score": score,
            "grade": grade,
            "rows": int(len(self.frame)),
            ...
            "metadata_completeness": round(
                (len(REQUIRED_METADATA_FIELDS) - len(missing_metadata)) / len(REQUIRED_METADATA_FIELDS), 4
            ),
    ...
    def drift_report(self) -> dict[str, Any]:
        ...
        for column in sorted(DRIFT_COLUMNS & set(self.frame.columns)):
            ...
            pooled_std = math.sqrt(max(0.0, (float(train_values.var(ddof=0)) + float(test_values.var(ddof=0))) / 2))
...
def list_datasets() -> list[dict[str, Any]]:
    ...
    for csv_path in sorted(DATASET_DIR.glob("*.csv")):
        try:
            ...
            items.append({"id": csv_path.stem, "path": str(csv_path), "valid": False, "error": str(exc)})
    return items
