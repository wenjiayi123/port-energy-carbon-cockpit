...
from app.rl.dataset import DATASET_DIR, PortDataset, list_datasets, portable_dataset_reference
...
class TrainingService:
    ...
    def validate_config(self, raw: dict[str, Any]) -> dict[str, Any]:
        ...
        dataset_value = str(raw.get("dataset_id") or raw.get("data_file") or "port_la_2025_monthly")
    ...
    def evaluate(self, strategy_id: str = "auto:latest") -> dict[str, Any]:
        ...
        for row_index in test_start_indices:
            ...
            learned_summary = self._rollout(learned, model, algorithm, row_index, int(config["seed"]))
            baseline_summary = self._rollout(baseline, None, "mpc", row_index, int(config["seed"]))
    ...
    def _run_training(self) -> None:
        ...
        try:
            ...
            model = self._build_model(config, env)
    ...
    def _build_model(self, config: dict[str, Any], env: PortEnergyDispatchEnv):
        ...
        if algorithm == "ppo":
            ...
            return PPO(**common, n_steps=n_steps, batch_size=batch_size, ent_coef=float(config.get("entropy_coef") or 0.0))
        if algorithm == "sac":
            return SAC(
        ...
        if algorithm == "td3":
            noise = NormalActionNoise(mean=np.zeros(3), sigma=0.1 * np.ones(3))
            return TD3(
    ...
    def _validation_rollout(self, model: Any, config: dict[str, Any], step: int) -> dict[str, Any]:
        ...
        summary = self._rollout(env, model, config["algorithm"], step % len(env.frame), int(config["seed"]))
        return {
    ...
    def _complete_mpc_job(self) -> None:
        ...
        artifact_path.write_text(
            json.dumps({"algorithm": "mpc", "controller": "three-step constrained MPC beam search over 27 actions", "config": self._job["config"]}, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
    ...
    def _rollout(self, env: PortEnergyDispatchEnv, model: Any, algorithm: str, row_index: int, seed: int) -> dict[str, Any]:
