...
| P1 | API-selected dataset paths and raw exception text were reported by CodeQL | A crafted path or internal error could expose local files and implementation details | HTTP datasets now resolve through a trusted server-side registry; API-key comparison uses constant-time bytes without secret fingerprints; client errors use stable public codes while detailed traces remain server-side | Re-run CodeQL after operator visual acceptance and before the release commit |
| P1 | The dependency graph contained vulnerable transitive frontend packages, and Intel macOS could only resolve an obsolete PyTorch wheel | Feature tests could pass while carrying known supply-chain risk | Pinned patched PostCSS/Nanoid overrides, upgraded the data cryptography extra, separated neural RL from the integration plane, and require an audited Linux/Apple-Silicon learner runtime | Intel macOS remains unsupported for neural training; GitHub Linux CI defines the supported security baseline |
| P1 | Container base images were mutable tags and workflow write permission was global | Upstream tag drift or an over-privileged job could weaken reproducibility | Python, Node and Nginx bases are digest-pinned; CodeQL upload permission is scoped to its analysis job | Refresh image digests through reviewed dependency updates; Python hash-locking remains a release follow-up |
