...
from app.schemas.electrical_network import (  # noqa: E402
    SOURCE_DOMAINS,
    ElectricalNetworkAssessmentRequest,
)
from app.services.electrical_network import (  # noqa: E402
    canonical_sha256,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Sign exactly one electrical evidence domain in a complete network "
            "assessment request with its source owner's Ed25519 private key."
        )
    )
    parser.add_argument("--input", required=True, help="Complete assessment request JSON")
    ...
    parser.add_argument("--output", required=True, help="Updated assessment request JSON")
    ...
    request = ElectricalNetworkAssessmentRequest(**payload)
    signed_payload_sha256 = canonical_sha256(source_domain_payload(request, args.domain))
    ...
    ElectricalNetworkAssessmentRequest(**payload)
