...
from app.schemas.measurement_verification import MeasurementVerificationRequest  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Sign an energy-carbon M&V evidence package with an Ed25519 private key."
    )
    ...
    parser.add_argument("--key-id", required=True, help="Trusted verifier key ID")
    ...
    independent = dict(payload.get("independent_verification") or {})
    independent["key_id"] = args.key_id
    independent["signed_evidence_sha256"] = "0" * 64
    independent["signature"] = base64.b64encode(b"0" * 64).decode("ascii")
    payload["independent_verification"] = independent
    ...
    MeasurementVerificationRequest(**payload)
