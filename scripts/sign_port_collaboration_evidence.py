...
from app.schemas.port_collaboration import (  # noqa: E402
    SOURCE_DOMAINS,
    PortCollaborationRequest,
)
from app.services.port_collaboration import (  # noqa: E402
    canonical_sha256,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Sign all seven port-collaboration source domains with Ed25519 keys."
    )
    ...
    parser.add_argument(
        ...
        help="Repeat once for each of the seven source domains",
    )
    parser.add_argument(
        "--key-id",
        action="append",
    ...
    provisional = PortCollaborationRequest(**payload)
    ...
    payload["source_attestations"] = [attestations[item] for item in sorted(SOURCE_DOMAINS)]
    PortCollaborationRequest(**payload)
        ...
        json.dumps(
            {
                ...
                "port_collaboration_public_keys_json": json.dumps(
