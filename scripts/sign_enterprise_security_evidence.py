...
from app.schemas.enterprise_security import (  # noqa: E402
    SOURCE_DOMAINS,
    EnterpriseSecurityRequest,
)
from app.services.enterprise_security import (  # noqa: E402
    canonical_sha256,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Sign all nine enterprise/OT-security source domains with Ed25519 keys."
    )
    ...
    parser.add_argument(
        ...
        help="Repeat once for each of the nine source domains",
    )
    parser.add_argument(
        "--key-id",
        action="append",
    ...
    payload["source_attestations"] = [attestations[item] for item in sorted(SOURCE_DOMAINS)]
    EnterpriseSecurityRequest(**payload)
        ...
        json.dumps(
            {
                ...
                "enterprise_security_public_keys_json": json.dumps(
