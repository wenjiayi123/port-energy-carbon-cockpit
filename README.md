...
**2026-09-07 · v7 新增离线准入策略。** 三随机种子各完成 115,200 步策略搜索训练并达到预设验证平台期；在 96 个互不重叠的 24 小时回放窗口上，三组均通过业务准入。当前策略由验证集选定为种子 37，模型、失败实验和历史版本证据均保留。

**业务价值：** 在考虑海事检查、海关查验、监管等待及放行后作业恢复等因素的前提下，当前策略相对因果模型预测控制仍实现**降低情景费用 1.11%**，平均每 24 小时情景节省综合费用约 **8,522 元**；**降低作业延误 6.21%**、**减少岸桥任务逾期量 19.15%**、**降低碳排 0.109%**，同时保持吞吐及岸电服务不退化，测试场景安全越界为零。相对验证集选出的强固定策略，仍额外降低费用 **0.338%**，平均每 24 小时情景额外节省约 **2,574 元**（费用降幅的 95% 配对置信区间为 **0.282%–0.395%**）。金额包括电费、燃油、延误等模拟成本及终态补能结算，属于公开数据校准的回顾性情景收益。监管字段为工程建模，检查、扣留和正式放行由外生事件驱动；现场数据接入与独立收益验收尚未完成，生产控制继续关闭。

已实际完成 **72 小时“模型预测控制 → 学习策略 → 回退”连续回放**，两次切换均保留储能电量、作业与监管队列，安全越界为零。见 [训练诊断、业务价值与切换说明](docs/DISPATCH_V7_CONVERGENCE_AND_CUTOVER_2026-09-07.md)、[最终机器报告](reports/dispatch_v7_qualified_attempt_01/business_value.json)、[收敛曲线](reports/dispatch_v7_attempt_03/convergence.png) 和 [切换验收回执](reports/dispatch_v7_cutover_acceptance_2026-09-07.json)。
...
| 当前正式结论 | v7 的 CEM 学习策略与强参照保护组合通过三种子离线业务准入，当前按验证集选定种子 37。历史 v6 仍为 `no_rl_policy_admitted`，其失败记录与历史 v4 碳排 `-8.792%` 等指标继续冻结保留；各版本均不能代替现场 KPI。 |
